...
type
  ...
  private
    ...
    FIsTopMost: Boolean;
    FCurrentActionBtn: TButton;
    FCurrentActionType: string;
    FResponseProcessed: Boolean;
    FUpdatingInProgress: Boolean;
    FDeviceGUID: string;
    FCurrentRequestID: string;  // 当前正在等待响应的 RequestID
    procedure LoadOrCreateGUID;
    ...
    procedure CheckForUpdate(ShowDialog: Boolean = False);
    ...
    function GetVersionLevel(const CurrentVer, NewVer: string): Integer;
...
// ================== GUID 管理 ==================
procedure Tmainform.LoadOrCreateGUID;
var
  GUIDStr: string;
begin
  GUIDStr := LoadConfigFromFile('Device', 'GUID', '');
  if GUIDStr = '' then
  begin
...
// ================== JSON 消息构建函数 ==================
function BuildMessageJSON(const DeviceGUID, Username, Message, Duration, DisplayMode, RequestID: string): string;
var
  JSON: TJSONObject;
begin
...
function BuildCloseJSON(const DeviceGUID, Username, RequestID: string): string;
var
  JSON: TJSONObject;
begin
  JSON := TJSONObject.Create;
...
function BuildCmdJSON(const DeviceGUID, Username, Command, Permission, RequestID: string): string;
var
  JSON: TJSONObject;
begin
  JSON := TJSONObject.Create;
...
// ================== JSON 辅助函数 ==================
function LoadJSONConfig: TJSONObject;
var
  FilePath: string;
  Content: string;
begin
  FilePath := mainform.GetAppConfigPath;
...
procedure SaveJSONConfig(JSON: TJSONObject);
var
  FilePath: string;
begin
  FilePath := mainform.GetAppConfigPath;
...
begin
  JSON := LoadJSONConfig;
  try
    if not JSON.TryGetValue<TJSONObject>(Section, SectionObj) then
    begin
...
begin
  ...
  JSON := LoadJSONConfig;
  try
    if JSON.TryGetValue<TJSONObject>(Section, SectionObj) then
    begin
      if SectionObj.TryGetValue<string>(Key, Value) then
        Result := Value;
    end;
  finally
    JSON.Free;
...
begin
  ...
  try
    ...
    JSON := LoadJSONConfig;
    try
      if JSON.TryGetValue<TJSONObject>('tips', TipsObj) then
      begin
        for i := 0 to TipsObj.Count - 1 do
        begin
          Pair := TipsObj.Pairs[i];
...
var
  i: Integer;
  OldSendText, OldEndText, OldCmdText: string;
begin
  OldSendText := sendIPtext.Text;
  OldEndText := endIPtext.Text;
  OldCmdText := cmdIPtext.Text;
  ...
  if OldSendText <> '' then
  begin
    i := sendIPtext.Items.IndexOf(OldSendText);
    if i >= 0 then
      sendIPtext.ItemIndex := i
    else
  ...
  if OldEndText <> '' then
  begin
    i := endIPtext.Items.IndexOf(OldEndText);
    if i >= 0 then
      endIPtext.ItemIndex := i
    else
  ...
  if OldCmdText <> '' then
  begin
    i := cmdIPtext.Items.IndexOf(OldCmdText);
    if i >= 0 then
      cmdIPtext.ItemIndex := i
    else
...
begin
  JSON := LoadJSONConfig;
  try
    if JSON.GetValue('tips') <> nil then
      JSON.RemovePair('tips');

    TipsObj := TJSONObject.Create;
    for i := 0 to FIPAliases.Count - 1 do
...
begin
  ...
  // 读取置顶设置（内联原 LoadTopMostSetting）
  TopMostStr := LoadConfigFromFile('Window', 'TopMost', '');
  ...
  CountStr := LoadConfigFromFile('Update', 'StartupCount', '0');
  StartupCount := StrToIntDef(CountStr, 0);
  ...
  // 读取 Skia 设置（内联原 LoadSkiaSetting）
  var SkiaStr := LoadConfigFromFile('Graphics', 'SkiaEnabled', '');
...
var
  ReceivedText: string;
  Lines: TArray<string>;
  RequestID, Status, ErrorMsg: string;
  i: Integer;
  Count: Integer;
begin
  ...
  // 按行分割
  Lines := ReceivedText.Split([#10]);

  if Length(Lines) < 2 then
    Exit;

  // === 第1行：RequestID ===
  RequestID := Trim(Lines[0]);

  if RequestID <> FCurrentRequestID then
    Exit;
  ...
  // ========== 结束消息特殊处理 ==========
  if FCurrentActionType = 'end' then
  begin
    if Status = 'ERR' then
    begin
      // 直接取第2行之后的所有内容作为错误信息（保持原始换行）
      ErrorMsg := '';
      for i := 2 to Length(Lines) - 1 do
      begin
        if ErrorMsg <> '' then
          ErrorMsg := ErrorMsg + #10;  // 用 #10 而不是 sLineBreak
    ...
    if TryStrToInt(Status, Count) then
    begin
      FResponseProcessed := True;
...
begin
  ...
  if PublishDate <> '' then
    DialogMsg := DialogMsg + '发布时间: ' + PublishDate + #13#10#13#10;
