...
namespace 远程控制平台命令执行端
{
    public class 远程控制平台命令执行端 : ServiceBase
    {
        ...
        private void ListenForCommands()
        {
            ...
            while (isListening)
            {
                try
                {
                    ...
                    if (messageType == "cmd")
                    {
                        string username = jsonObj.ContainsKey("username") ? jsonObj["username"].ToString() : "未知用户";
                        string command = jsonObj.ContainsKey("command") ? jsonObj["command"].ToString() : "";
                        string permission = jsonObj.ContainsKey("permission") ? jsonObj["permission"].ToString() : "SYSTEM";

                        EventLog.WriteEntry("收到CMD命令 (用户: " + username + ", 权限: " + permission + "): " + command, EventLogEntryType.Information);
                        ExecuteCmdCommand(remoteEP.Address.ToString(), command, remoteEP.Port, requestID);
                    }
                }
        ...
        // ================== 执行CMD命令 ==================
        private void ExecuteCmdCommand(string senderIP, string command, int remotePort, string requestID)
        {
            ThreadPool.QueueUserWorkItem(state =>
            {
                        ...
                        if (isGuiCommand)
                        {
                            if (process.WaitForExit(2500))
                            {
                                ...
                                if (process.ExitCode != 0 && !string.IsNullOrWhiteSpace(error))
                                {
                                    ...
                                    SendResponse(senderIP, requestID, fullError, remotePort, true);
                                    EventLog.WriteEntry("CMD命令执行失败: " + command + ", 错误: " + error, EventLogEntryType.Error);
                                    return;
                                }
                            }
                            
                            SendResponse(senderIP, requestID, "", remotePort, false);
                ...
                {
                    SendResponse(senderIP, requestID, "执行CMD命令时出错\n错误信息: " + ex.Message, remotePort, true);
        ...
        // 成功：{requestID}\nOK\n
        // 错误：{requestID}\nERR\n{errorMessage}\n
        private void SendResponse(string senderIP, string requestID, string message, int remotePort, bool isError)
        {
            try
            {
