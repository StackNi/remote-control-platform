    ...
    try
      try
        ...
        begin
          ...
          if (ChildControl.Align in HorizontalAlignments) and CanFlipChild(ChildControl) then
...
procedure TControl.ResetFocus;
  ...
  begin
    Result := (Root.Focused <> nil) and (Root.Focused.GetObject = Self);
  end;
...
begin
  ...
  // When form is being deactivated, it reset locally focus in control. However, it keeps the reference in Root.Focused
  // for future setting focus in form activation. So if user would like to reset focus between Deactivation and Activation,
  // we should let him do it.
  if FIsFocused or IsFocusedInForm then
    Root.SetFocused(nil)
  else
...
begin
  // 添加左键检查，并检查被按下的控件是不是自己
  if AbsoluteEnabled and FPressed and not FDoubleClick and PointInObjectLocal(X, Y) and (Button = TMouseButton.mbLeft) and (LastDown = Self) then
  begin
    Click;
    FPressed := False;
    StartTriggerAnimation(Self, 'Pressed');
...
begin
  if not SameValue(FRotationAngle, Value, TEpsilon.Angle) then
  begin
...
begin
  if not SameValue(FOpacity, Value, TEpsilon.Scale) then
  begin
...
var
  ...
  SaveDisableAlign: Boolean;
begin
  if FIsNeedStyleLookup and (Scene <> nil) and (not InPaintTo and (FResourceLink = nil)) then
  begin
...
procedure TStyledControl.ScaleChangedHandler(const Sender: TObject; const Msg: System.Messaging.TMessage);
  ...
  function IsControlForm: Boolean;
  begin
    Result := (Root <> nil) and (TScaleChangedMessage(Msg).Value = Root.GetObject);
  end;

begin
  if IsControlForm then
    DoStyleChanged;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      if TFmxFormState.Released in TOpenForm(FPopupForm).FormState then
        FPopupForm := nil;
    end;
  end
  else
...
begin
  if not SameValue(FBorderWidth, Value, TEpsilon.Position) then
  begin
