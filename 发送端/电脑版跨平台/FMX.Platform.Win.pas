...
// procedure PlatformMinimizeWindow(Form: TCommonCustomForm);
...
type
  ...
  private
	...
	// procedure MinimizeWindowEx(const AForm: TCommonCustomForm);
...
begin
  ...
  begin
	...
	// 刷新一下 ApplicationHWND
	Winapi.Windows.ShowWindow(ApplicationHWND, SW_SHOWMINIMIZED);
    Winapi.Windows.ShowWindow(ApplicationHWND, SW_SHOWNORMAL);
    case CmdShow of
      SW_SHOWMINNOACTIVE:
...
{// 自制最小化
procedure TPlatformWin.MinimizeWindowEx(const AForm: TCommonCustomForm);
var
  H: HWND;
begin
  ...
  H := WindowHandleToPlatform(AForm.Handle).Wnd;
...
begin
  ...
  begin
    ...
    begin
      case uMsg of
    // 在这里添加对 WM_ACTIVATE、WM_SETFOCUS、WM_NCACTIVATE、WM_MOUSEACTIVATE 的处理
	// 效仿 WM_ACTIVATEAPP，激活时调用 RestoreApp
    WM_ACTIVATE:
      begin
        Result := DefWindowProc(hwnd, uMsg, wParam, lParam);
        if (LOWORD(wParam) <> 0) then
...
procedure TPlatformWin.RestoreApp;
  ...
  function GetTopmostVisibleForm: TCommonCustomForm;
  var
    I: Integer;
    Form: TCommonCustomForm;
    FormHwnd: HWND;
  begin
    ...
    begin
      Form := Screen.Forms[I];
      if Form.Visible and (Form.FormStyle <> TFormStyle.Popup) then
      begin
        FormHwnd := FormToHWND(Form);
        if (FormHwnd <> 0) and IsWindowVisible(FormHwnd) and not IsIconic(FormHwnd) then
        begin
          Result := Form;
          Break;
        end;
...
begin
  var Form := GetTopmostVisibleForm;
  if Form = nil then
  begin
    // 没有可见表单，尝试激活主表单
    if Application.MainForm <> nil then
  ...
  var FormHwnd := FormToHWND(Form);
  if FormHwnd = 0 then Exit;
  
  // 直接激活窗口，不依赖 FMX 的 Activate
  if IsWindowVisible(FormHwnd) and not IsIconic(FormHwnd) then
  begin
    // 使用 Windows API 直接激活
    SetActiveWindow(FormHwnd);
  end;
end;
...
procedure PlatformMinimizeWindow(Form: TCommonCustomForm);
begin
  if (Form <> nil) and Form.IsHandleAllocated and 
     (PlatformWin <> nil) then
  begin
    PlatformWin.MinimizeWindowEx(Form);
  end;
end;}
