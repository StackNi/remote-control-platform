...
type
  ...
  TScrollBarHelper = class helper for TScrollBar
    function GetTrack: TCustomTrack;
    property Track: TCustomTrack read GetTrack;
  end;
...
procedure TStyledCustomScrollBox.FreeStyle;
  ...
  begin
    ...
    begin
      ...
      AScrollInfo.Scroll.OnChange := nil;
...
begin
  ...
  if not Model.IsOpaque then
    UpdateEffects;
  // 滑块被拖拽时不触发惯性动画系统的状态更新
  if (HScrollBar = nil) or (HScrollBar.Track = nil) or (HScrollBar.Track.Thumb = nil) or not HScrollBar.Track.Thumb.IsPressed then
...
begin
  ...
  if not Model.IsOpaque then
    UpdateEffects;
  // 滑块被拖拽时不触发惯性动画系统的状态更新
  if (VScrollBar = nil) or (VScrollBar.Track = nil) or (VScrollBar.Track.Thumb = nil) or not VScrollBar.Track.Thumb.IsPressed then
    FAniCalculations.Shown := True;
end;
...
begin
  if InInternalAlign then
    Exit;

  if csLoading in ComponentState then
    FContent.Size.Size := Message.Value.Size
  else
  begin
...
function TScrollBarHelper.GetTrack: TCustomTrack;
begin
  Result := TCustomTrack(FindStyleResource('htrack'));
  if Result = nil then
    Result := TCustomTrack(FindStyleResource('vtrack'));
  if Result = nil then
    Result := TCustomTrack(FindStyleResource('track'));
end;
