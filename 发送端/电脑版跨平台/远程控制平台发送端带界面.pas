...
uses
  ...
  System.JSON
  {$IFDEF MSWINDOWS}
  , FMX.Platform.Win, Winapi.Windows, Winapi.Messages, Winapi.ShellAPI, Winapi.WinInet
  {$ENDIF}
  {$IFDEF MACOS}
...
type
  ...
  private
    ...
    procedure ShowUpdateDialog(const VersionInfo: string; const UpdateNotes: string;
                               const DownloadUrl: string; const UpdateLevel: Integer);
...
var
  DialogMsg: string;
begin
  DialogMsg := '发现新版本: ' + VersionInfo + #13#10 +
               '更新内容: ' + UpdateNotes + #13#10#13#10;

  if UpdateLevel = 1 then
    DialogMsg := DialogMsg + '本次更新为架构层面升级，建议立即更新！'
  else if UpdateLevel = 2 then
    DialogMsg := DialogMsg + '建议更新以获得更好的体验。'
  else
    DialogMsg := DialogMsg + '是否现在更新？';

  if UpdateLevel = 1 then
  begin
    TDialogService.MessageDialog(DialogMsg, TMsgDlgType.mtWarning,
  ...
  try
    ...
    try
      ...
      begin
        ...
        begin
          // 主版本更新：强制弹窗
          ShowUpdateDialog(NewVersion, UpdateNotes, DownloadUrl, UpdateLevel);
        end
        else if UpdateLevel = 2 then
        begin
          // 次版本更新：判断启动计数是否 ≥8
...
begin
  ...
  begin
    try
      hSession := InternetOpen('RemoteControl-Windows/' + APP_VERSION,
                               INTERNET_OPEN_TYPE_DIRECT,
                               nil, nil, 0);
      if hSession = nil then
      begin
        LastError := GetLastError;
        raise Exception.Create('InternetOpen 失败, 错误码: ' + IntToStr(LastError));
      end;
      ...
      try
        ...
        hURL := InternetOpenUrl(hSession,
          PChar('https://stackni.github.io/remote-control-platform/version.json'),
          nil, 0,
          INTERNET_FLAG_RELOAD or INTERNET_FLAG_NO_CACHE_WRITE or
          INTERNET_FLAG_PRAGMA_NOCACHE or INTERNET_FLAG_DONT_CACHE or INTERNET_FLAG_SECURE,
          0);

        if hURL = nil then
        begin
          LastError := GetLastError;
          raise Exception.Create('InternetOpenUrl 失败, 错误码: ' + IntToStr(LastError));
        end;
        ...
        try
          ...
          try
            repeat
              ...
              begin
                LastError := GetLastError;
                raise Exception.Create('InternetReadFile 失败, 错误码: ' + IntToStr(LastError));
              end;
              if BytesRead = 0 then Break;
              Stream.WriteBuffer(Buffer, BytesRead);
            until False;

            SetLength(UTF8Bytes, Stream.Size);
            Move(Stream.Memory^, UTF8Bytes[0], Stream.Size);
        ...
        begin
          if ShowDialog then
            TThread.Queue(nil,
              procedure
              begin
                TDialogService.MessageDialog('检查更新失败: ' + E.Message,
                  TMsgDlgType.mtError, [TMsgDlgBtn.mbOK], TMsgDlgBtn.mbOK, 0, nil);
              end);
          Break;
        end;
...
begin
  ...
  TThread.Queue(nil,
    ...
    begin
      if ShowDialog then
        TDialogService.MessageDialog('检查更新功能仅支持 Windows 平台',
          TMsgDlgType.mtInformation, [TMsgDlgBtn.mbOK], TMsgDlgBtn.mbOK, 0, nil);
      FUpdatingInProgress := False;
    end);
end;
