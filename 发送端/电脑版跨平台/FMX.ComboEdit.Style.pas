...
begin
  ...
  begin
    ...
    // ============== 在这里添加 Hint 设置 ==============
    FArrowButton.Hint := '从现有IP提示词中选择';  // 设置下拉按钮的提示
    FArrowButton.ShowHint := True;       // 启用提示显示
    // ================================================
	// 改变下拉按钮尺寸
    FArrowButton.Width := 24;
    
    // 设置触摸目标扩展（如果有触摸屏）
    if SupportsPlatformService(IFMXDeviceService, IInterface(DeviceSrv)) and 
       (TDeviceFeature.HasTouchScreen in DeviceSrv.GetFeatures) then
      FArrowButton.TouchTargetExpansion.Rect := 
        TRectF.Create(0, DefaultTouchTargetExpansion, DefaultTouchTargetExpansion, 0)
  end;
end;
